...
% categories defined by T. Because the two kinds of input vectors cannot be separated 
% by a straight line, the perceptron will not be able to do it.
% 
% Here the initial perceptron is created and configured. (The configuration 
% step is normally optional, as it is performed automatically by ADAPT and TRAIN.)
%%
net = perceptron;
net = configure(net,X,T);
%% 
% Add the neuron's initial attempt at classification to the plot. The initial 
% weights are set to zero, so any input gives the same output and the classification 
% line does not even appear on the plot.
%%
hold on
plotpv(X,T);
...
% the outputs and error. The loop allows the network to repeatedly adapt, plots 
% the classification line, and stops after 25 iterations.
%%
for a = 1:100
...
%% 
% Note that zero error was never obtained. Despite training, the perceptron 
% has not become an acceptable classifier. Only being able to classify linearly 
% separable data is the fundamental limitation of perceptrons.
% 
% _Copyright 1992-2011 The MathWorks, Inc._